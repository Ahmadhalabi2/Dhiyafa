 ...
 * يرسل إيميل تأكيد حجز بتصميم HTML احترافي (مدمج بالكامل Inline)
 * ─────────────────────────────────────────────────────
 */
...
// ── تحقق هل الإيميل من نوع يدعم الإرسال ─────────────────────────────────
...
function buildConfirmationHtml(booking) {
  ...
  const extrasRows = (extras && extras.length > 0)
    ? extras.map((ex) => `
        <tr>
          <td style="padding:6px 12px;border-bottom:1px solid #E5DFC8;font-size:12px;color:#52655F;text-align:right;">${ex.label}</td>
          <td style="padding:6px 12px;border-bottom:1px solid #E5DFC8;font-size:12px;color:#1C2B27;text-align:left;font-weight:700;"><bdi dir="ltr">$${ex.amount?.toLocaleString() ?? '—'}</bdi></td>
        </tr>`).join('')
    : `<tr><td colspan="2" style="padding:8px 12px;font-size:11px;color:#93A29B;text-align:center;">لا توجد إضافات</td></tr>`;
...
<body style="margin:0;padding:0;background-color:#F3EEE1;font-family:'Segoe UI',Tahoma,Arial,sans-serif;direction:rtl;">
  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#F3EEE1;padding:20px 10px;">
    <tr>
      <td align="center">
        <table width="480" cellpadding="0" cellspacing="0" border="0"
               style="width:100%;max-width:480px;background-color:#FFFFFF;border-radius:16px;overflow:hidden;
          ...
          <tr>
            <td style="background-color:#0E5C4A;background:linear-gradient(135deg,#0E5C4A 0%,#0A4437 100%);
              ...
              <div style="display:inline-block;background-color:rgba(255,255,255,0.15);
                          border-radius:10px;padding:5px 14px;margin-bottom:8px;">
          ...
          <tr>
            <td style="padding:24px 28px;">
              <p style="color:#152A24;font-size:15px;margin:0 0 6px;font-weight:600;direction:rtl;text-align:right;">
                أهلاً <bdi dir="ltr" style="display:inline-block;">${userName}</bdi>،
              </p>
              <p style="color:#4A5568;font-size:12px;margin:0 0 18px;line-height:1.7;text-align:right;">
              ...
              <div style="background-color:#F8F4EA;background:linear-gradient(135deg,#F8F4EA 0%,#EDE6D6 100%);
                          border:2px solid #C69A3A;border-radius:12px;
                          padding:14px 12px;text-align:center;margin-bottom:18px;">
              ...
              <table width="100%" cellpadding="0" cellspacing="0" border="0"
                style="background-color:#F8F4EA;border-radius:10px;border:1px solid #E5DFC8;margin-bottom:18px;">
                <tr>
                  <td style="padding:12px 14px;border-bottom:1px solid #EDE6D6;text-align:right;" colspan="2">
                    <p style="margin:0 0 2px;font-size:10px;color:#93A29B;font-weight:700;">الفندق</p>
                    <p style="margin:0;font-size:14px;font-weight:700;color:#1C2B27;direction:rtl;"><bdi>${hotelName}</bdi></p>
                    <p style="margin:2px 0 0;font-size:11px;color:#52655F;">${city}، ${country}</p>
                  </td>
                </tr>
                <tr>
                  <td style="padding:10px 14px;border-bottom:1px solid #EDE6D6;width:50%;border-left:1px solid #EDE6D6;text-align:right;">
                    <p style="margin:0 0 2px;font-size:10px;color:#93A29B;">تسجيل الوصول</p>
                    <p style="margin:0;font-size:13px;font-weight:700;color:#0E5C4A;direction:ltr;text-align:right;">${checkIn}</p>
                  </td>
                  <td style="padding:10px 14px;border-bottom:1px solid #EDE6D6;width:50%;text-align:right;">
                    <p style="margin:0 0 2px;font-size:10px;color:#93A29B;">تسجيل المغادرة</p>
                    <p style="margin:0;font-size:13px;font-weight:700;color:#BD5B3E;direction:ltr;text-align:right;">${checkOut}</p>
                  </td>
                </tr>
                <tr>
                  <td style="padding:10px 14px;border-left:1px solid #EDE6D6;text-align:right;">
                    <p style="margin:0 0 2px;font-size:10px;color:#93A29B;">عدد الليالي</p>
                    <p style="margin:0;font-size:13px;font-weight:700;color:#1C2B27;">${nights} ليلة</p>
                  </td>
                  <td style="padding:10px 14px;text-align:right;">
                    <p style="margin:0 0 2px;font-size:10px;color:#93A29B;">عدد الضيوف</p>
                    <p style="margin:0;font-size:13px;font-weight:700;color:#1C2B27;">${guests} ضيف</p>
                  </td>
                </tr>
                <tr>
                  <td style="padding:10px 14px;border-top:1px solid #EDE6D6;text-align:right;" colspan="2">
              ...
              <table width="100%" cellpadding="0" cellspacing="0" border="0"
                style="border:1px solid #E5DFC8;border-radius:8px;margin-bottom:18px;">
                ...
                <table width="100%" cellpadding="0" cellspacing="0" border="0">
                  <tr>
                    <td style="text-align:right;">
                      ...
                      <p style="margin:2px 0 0;font-size:20px;font-weight:800;color:#FFFFFF;"><bdi dir="ltr">$${amount.toLocaleString()}</bdi></p>
                      <p style="margin:2px 0 0;font-size:10px;color:#E8C766;">يعادل تقريباً ${formatSYP(amount)} ل.س</p>
                    </td>
                    <td style="text-align:left;vertical-align:middle;">
                      <div style="background-color:rgba(255,255,255,0.15);border-radius:6px;padding:5px 10px;display:inline-block;">
                        <span style="color:#FFFFFF;font-size:11px;font-weight:700;">✅ مدفوع ومؤكد</span>
                      </div>
                    </td>
                  </tr>
                </table>
              ...
              <div style="background-color:#FFF8EC;border:1px solid #F5D87A;border-radius:10px;
                          padding:10px 14px;">
                <p style="margin:0;font-size:11px;color:#7A5A00;line-height:1.6;text-align:right;">
          ...
          <!-- Footer -->
          <tr>
            <td style="background-color:#F8F4EA;padding:14px 28px;
...
router.post('/send-confirmation', requireAuth, async (req, res) => {
  ...
  try {
    ...
    await transporter.sendMail({
      from: `"${process.env.MAIL_FROM_NAME || 'ضيافة - Dhiyafa'}" <${process.env.MAIL_USER}>`,
      to:   userEmail,
